...
void CollisionSystem::terrainToFluidCollision(Terrain& terrain, FluidSystem& fluidSystem, f32 dt) {
    ...
    auto buildGrid = [&]() {
        ...
        // Loop each particle type (e.g. water, lava) and POPULATE the grid with the indices of
...
bool CollisionSystem::detectCircleVsAABB(const AEVec2& circleCenter, f32 radius,
    ...
    if (isInside) {
        ...
        if (distToEdgeX < distToEdgeY) {
            ...
            // Push along the X axis towards the closest edge
            ...
            // center....
            // Eject against vertical velocity — fallback to position if stationary
            // Push along the Y axis towards the closest edge
...
void CollisionSystem::pushOutAndSlide(FluidParticle& p, const AEVec2& n, f32 penetration,
    ...
    if (vn < 0.0f) {
        ...
        if (n.y <= 0.5f) {
            ...
            f32 microBump = (rand() % 100) * 0.15f * (dt / 0.004f); // Scale with substep dt
            microBump = (std::min)(microBump, MAX_MICRO_BUMP);      // Hard cap
...
void CollisionSystem::resolveFluidParticlePair(FluidParticle& p1, FluidParticle& p2) {
    ...
    if (distSq < minDist * minDist) {
        ...
        // Clamp only upward direction (positive Y)
